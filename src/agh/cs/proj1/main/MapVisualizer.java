...
public class MapVisualizer {
    ...
    private String drawObject(Vector2d currentPosition) {
        String result = null;
        if (this.map.isOccupiedByAnimals(currentPosition)) {
            Object object = null;
            LinkedList<Animal> animals = this.map.animalsAt(currentPosition);
            object = animals.getFirst();
            result = object.toString();
        }
        else if(this.map.isGrassAtPosition(currentPosition)){
            Object object = this.map.getGrassFromPosition(currentPosition);
            result = object.toString();
        }
        else {
            result = EMPTY_CELL;
